...
- Put verified Duel Deck tokens in the `Sideboard` section of the corresponding real deck, marked `[token]`; do not create separate token-only decks or sealed deck references for them. Assign tokens to the deck whose cards create them, preserve product totals and exact art variants, and handle alternate editions such as Japanese decks and Anthology decks explicitly. Sealed products inherit these tokens through their existing real-deck references; remove redundant standalone-token references or descriptions. Verify quantities, finishes and physical faces; if ownership or collation is unknown, record the gap instead of guessing.
- Validate token references through the actual token UUID exporter using exact token-set codes and collector numbers. For example, `TDD2:1` resolves while `DD2:T1` does not. Matching names after stripping a `T` prefix is insufficient. Confirm every token has a unique usable UUID, and keep tokens excluded from gameplay card counts.
